...
class AnalysisProvider extends ChangeNotifier {
  ...
  Future<List<FoodAnalysis>> analyzeImage(String imagePath) async {
    ...
    try {
      // 1. Optimize Image
      final optimizedPath = await ImageOptimizer.compressImage(imagePath);
      
      _currentAnalysis = await _mlService.analyzeImage(optimizedPath);
      _isAnalyzing = false;
      notifyListeners();
    ...
    try {
      ...
      if (user != null) {
        await FirebaseFirestore.instance
          ...
          .collection('analyses')
          .add({
             'date': analysis.analyzedAt.toIso8601String().split('T')[0],
