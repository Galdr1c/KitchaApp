...
import 'dart:io';
import 'dart:async';
import 'recipe_mcp_service.dart';
import 'vision_mcp_service.dart';

/// Unified service for ML and API integrations
class AppService {
  static AppService _instance = AppService._internal();
  
  @visibleForTesting
  static set instance(AppService mock) => _instance = mock;
  
  final RecipeMcpService _recipeMcpService = RecipeMcpService();
  RecipeMcpService get recipeMcpService => _recipeMcpService;
  
  final VisionMcpService _visionMcpService = VisionMcpService();
  VisionMcpService get visionMcpService => _visionMcpService;

  // API Keys (loaded in initApiKeys)
  ...
  Future<void> initApiKeys() async {
    _spoonacularApiKey = dotenv.env['SPOONACULAR_API_KEY'];
    _nutritionixAppId = dotenv.env['NUTRITIONIX_APP_ID'];
    _nutritionixApiKey = dotenv.env['NUTRITIONIX_API_KEY'];

    if (!hasValidApiKeys) {
      print('[AppService] Warning: Missing API keys in .env file');
    }
  }
...
// ... 
  ...
  Future<RecipeSearchResult> fetchRecipes(String ingredients, {int number = 10}) async {
    ...
    try {
      if (!hasValidApiKeys) {
        // Fallback or demo mode logic
         return _getMockRecipeResults(ingredients);
      }
      
      // Try MCP first
      try {
        final mcpRecipes = await _recipeMcpService.searchRecipesByIngredients(ingredients);
        if (mcpRecipes.isNotEmpty) {
          // Sync with database as requested by legacy logic
          for (final recipe in mcpRecipes) {
            await _repository.insertRecipe(recipe.toRecipeModel());
          }
          return RecipeSearchResult(
            success: true, 
            recipes: mcpRecipes.map((r) => r.toRecipeModel()).toList(), 
            message: '${mcpRecipes.length} tarif bulundu (MCP)'
          );
        }
      } catch (e) {
        print('[AppService] MCP Search failed, falling back to legacy API: $e');
      }
      ...
      if (response.statusCode == 200) {
        // ... (parsing logic)
         ...
         return RecipeSearchResult(success: true, recipes: recipes, message: '${recipes.length} tarif bulundu');
      } else {
        throw AppException(
          message: 'API returned ${response.statusCode}',
          statusCode: response.statusCode,
          userFriendlyMessage: 'Tarifler alınamadı. Lütfen daha sonra tekrar deneyin.',
        );
      }
    } on SocketException {
      throw AppException(message: 'No Internet', userFriendlyMessage: 'İnternet bağlantısı yok.');
    } on TimeoutException {
      throw AppException(message: 'Timeout', userFriendlyMessage: 'Sunucu yanıt vermiyor. Lütfen bağlantınızı kontrol edin.');
    } catch (e) {
      print('[AppService] Recipe fetch error: $e');
      if (e is AppException) rethrow;
      throw AppException(message: e.toString(), userFriendlyMessage: 'Beklenmedik bir hata oluştu.');
    }
  }

  
  Future<List<RecipeModel>> getRandomRecipes({List<String>? tags, int number = 10}) async {
    if (_spoonacularApiKey == null || _spoonacularApiKey!.isEmpty) await initApiKeys();
    
    final tagsParam = tags != null && tags.isNotEmpty ? '&tags=${tags.join(",")}' : '';
    final url = Uri.parse('$_spoonacularBaseUrl/recipes/random?number=$number$tagsParam&apiKey=$_spoonacularApiKey');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (final r in recipesData) {
            recipes.add(_parseSpoonacularRecipe(r));
        }
        return recipes;
      } else {
        throw AppException(message: 'API Error: ${response.statusCode}', userFriendlyMessage: 'Rastgele tarifler alınamadı.');
      }
    } catch (e) {
      throw AppException(message: e.toString(), userFriendlyMessage: 'Beklenmedik bir hata oluştu.');
    }
  ...
  /// Get detailed nutrition for a recipe using MCP
  Future<Map<String, dynamic>> getRecipeNutrition(int recipeId) async {
    return await _recipeMcpService.getRecipeNutrition(recipeId);
  }

  /// Get daily menu suggestions using MCP
  Future<List<RecipeModel>> suggestDailyMenu({int? calories, String? diet}) async {
    final mcpRecipes = await _recipeMcpService.suggestDailyMenu(
      calories ?? 2000,
      diet != null ? {'diet': diet} : {},
    );
    return mcpRecipes.map((r) => r.toRecipeModel()).toList();
  }

  /// Find similar recipes using MCP
  Future<List<RecipeModel>> findSimilarRecipes(int recipeId) async {
    final mcpRecipes = await _recipeMcpService.findSimilarRecipes(recipeId);
    return mcpRecipes.map((r) => r.toRecipeModel()).toList();
  }
  ...
  /// Analyze image for food recognition (Multi-Model)
  Future<ImageAnalysisResult> analyzeImage(String photoPath) async {
    print('[AppService] Starting multi-model analysis for: $photoPath');
    
    try {
      // Use VisionMcpService for parallel analysis and model consensus
      final multiResult = await _visionMcpService.analyzeImageMultiModel(photoPath);
      
      if (multiResult.success && (multiResult.foods.isNotEmpty || multiResult.uncertainFoods.isNotEmpty)) {
        final allFoods = multiResult.foods + multiResult.uncertainFoods;
        final totalCalories = allFoods.fold(0, (sum, f) => sum + f.calories);
        ...
        print('[AppService] Multi-model analysis saved with ID: $id');
        ...
        return ImageAnalysisResult(
          success: true,
          foods: multiResult.foods,
          uncertainFoods: multiResult.uncertainFoods,
          modelUsed: multiResult.modelUsed,
          analysisId: id,
          message: '${multiResult.foods.length} yiyecek kesin, ${multiResult.uncertainFoods.length} şüpheli tespit edildi. (${multiResult.modelUsed})',
        );
      } else if (!multiResult.success) {
        throw Exception(multiResult.message ?? 'Multi-model analysis failed');
      }
      ...
      return ImageAnalysisResult(
        ...
        uncertainFoods: [],
        modelUsed: multiResult.modelUsed,
        message: 'Hiç yiyecek tespit edilemedi.',
      );
      
    } catch (e) {
      print('[AppService] Multi-model analysis error (falling back to local ML): $e');
      // Fallback to existing local ML Kit logic if multi-model fails logic
      return analyzeImageLocal(photoPath);
    }
  }

  /// Original local ML Kit analysis as fallback
      ...
      if (_imageLabeler != null && _objectDetector != null) {
        ...
        if (foodItems.isNotEmpty) {
          ...
          return ImageAnalysisResult(
            ...
            analysisId: id,
            message: '${foodItems.length} yiyecek tespit edildi (Yerel ML)',
          );
        }
      }
...
class ImageAnalysisResult {
  ...
  ImageAnalysisResult({
    ...
    required this.foods,
    this.uncertainFoods = const [],
    this.modelUsed = 'Google ML Kit',
    this.analysisId,
