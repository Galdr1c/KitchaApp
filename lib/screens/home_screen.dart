            ...
            SliverPersistentHeader(
              ...
              delegate: _SliverAppBarDelegate(
                TabBar(
                  ...
                  tabs: const [
                    Tab(text: 'Tarif Ara', icon: Icon(Icons.search)),
                    Tab(text: 'Kalori Hesapla', icon: Icon(Icons.calculate)),
                  ],
                ),
              ),
            ),
...
class _RecipeSearchTabState extends State<_RecipeSearchTab> {
  ...
  void initState() {
    ...
    // Default to Swipe/Discovery mode
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _toggleSurpriseMe();
    });
  }
  ...
  Future<void> _toggleSurpriseMe() async {
    setState(() {
      _isSwipeMode = true;
      _isSearching = false;
    ...
    await provider.getSurpriseRecipes();
  }
  
  Widget _buildRecipeSwiper(RecipeProvider provider) {
       ...
       return Center(
         child: Column(
           ...
           children: [
             ...
             const Text("Tarif bulunamadı!", style: TextStyle(fontSize: 18, color: Colors.grey)),
             const SizedBox(height: 16),
             ElevatedButton(
               onPressed: _toggleSurpriseMe,
    ...
    return Container(
      ...
      child: CardSwiper(
        ...
        cardsCount: provider.searchResults.length,
        onSwipe: (previousIndex, currentIndex, direction) {
          final recipe = provider.searchResults[previousIndex];
          if (direction == CardSwiperDirection.right) {
             provider.saveToFavorites(recipe);
             ScaffoldMessenger.of(context).showSnackBar(
               SnackBar(content: Text("${recipe.title} favorilere eklendi!"), duration: const Duration(seconds: 1)),
             );
          }
          return true;
        },
                     ...
                     child: Hero(
                        ...
                        child: ClipRRect(
                           ...
                           child: recipe.imageUrl.isNotEmpty
                             ...
                             : Container(color: Colors.grey[200], child: const Icon(Icons.restaurant, size: 64)),
                        ),
                     ),
                   ),
                   Expanded(
                     ...
                     child: Padding(
                       ...
                       child: Column(
                         ...
                         children: [
                           Text(recipe.title, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold), maxLines: 2, overflow: TextOverflow.ellipsis),
                           ...
                           const Spacer(),
                           const Center(child: Text("Detaylar için tıkla", style: TextStyle(color: Colors.blue))),
                         ],
                       ),
                     ),
                   ),
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        // Surprise Me Button
        if (!_isSwipeMode && !_isSearching && _lastQuery.isEmpty)
           Padding(
             ...
             child: SizedBox(
               ...
               child: ElevatedButton.icon(
                 ...
                 label: const Text("Beni Şaşırt (Rastgele Tarif)"),
        ...
        // Return to Search Button (when in Swipe Mode)
        if (_isSwipeMode)
           Padding(
             padding: const EdgeInsets.all(8.0),
             child: SizedBox(
        ...
        Expanded(
          child: _isSwipeMode 
             ? Consumer<RecipeProvider>(builder: (context, provider, _) => _buildRecipeSwiper(provider))
             : _buildRecipeList(),
        ),
      ],
    );
    ...
    try {
      ...
      if (mounted) {
        ...
        if (result.success && result.foods.isNotEmpty) {
          ...
          MemoryMcpService().updateMemoryFromAnalysis(analysis);
        }
    ...
    return Column(
      ...
      children: [
            ...
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    '${_currentResult!.modelUsed} kullanıldı',
                    style: const TextStyle(
                      fontSize: 11,
                      fontWeight: FontWeight.w600,
                      color: Colors.blue,
        ...
        ListView.builder(
          ...
          itemCount: (_currentResult?.foods.length ?? 0) + (_currentResult?.uncertainFoods.length ?? 0),
          itemBuilder: (context, index) {
            final isUncertain = index >= (_currentResult?.foods.length ?? 0);
            final food = isUncertain 
                ? _currentResult!.uncertainFoods[index - _currentResult!.foods.length]
                : _currentResult!.foods[index];
        ...
        child: Row(
          children: [
            ...
            Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(
                color: (isUncertain ? Colors.orange : Colors.green).withOpacity(0.1),
                borderRadius: BorderRadius.circular(8),
              ),
              child: Icon(
                isUncertain ? Icons.help_outline : Icons.restaurant,
                color: isUncertain ? Colors.orange : Colors.green,
                size: 20,
              ),
            ),
            const SizedBox(width: 12),
