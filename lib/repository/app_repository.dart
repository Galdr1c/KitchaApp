...
class AppRepository {
  ...
  Future<Database> _initDb() async {
    ...
    final password = await SecureStorageService().getDatabaseKey();

    return await openDatabase(
      path,
      password: password,
      version: 4, // Incrementing version to force reset/migration check if necessary
      onCreate: _onCreate,
  ...
  Future<void> _createAnalysesTable(Database db) async {
    await db.execute('''
  ...
  Future<void> _createIndexes(Database db) async {
     ...
     await db.execute('CREATE INDEX IF NOT EXISTS idx_analyses_date ON analyses(date)');
  }
